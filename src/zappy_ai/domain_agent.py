...
class Agent():
    ...
    def __init__(self, args):
        ...
        self.inventory = {
            ...
            "mendiane": 0,
            "phiras": 0,
            "thystame": 0}  # starting inventory updates
        self.ok_debt = 0
        self.level = 1  # level
        self.objects_countdown = None  # timeouts of seen parts of the map
    ...
    def starting_command(self, command: str):
        if self.starting == 0:
            if command == "BIENVENUE":
                ...
                print(
                    f"No se recibio el mensaje de bienvenida {command}")
                self.starting = 4
        if self.starting == 1:
            if command.isdigit():
                ...
                self.starting = 4
                print(
                    f"No se identifico el numero de clientes {command}")
        if self.starting == 2:
    ...
    def objects_cooldown(self):
        if self.objects_countdown is not None:
            ...
            # Based on subject 120 and evaluation sheet
            # get to level 8 in 10 minutes al speed 100
            substances_chances = {
                "nourriture": 0.5,
                "linemate": 0.3,
                "deraumere": 0.15,
            ...
            for key, value in substances_chances.items():
                ...
                if nplayers < nobj:
                    # assuming independance of picking up for each player
                    # random 0.25 for moving this from the edge case to the ez
                    # case
                    breaks = -(self.objects_countdown - self.turn) > 0.25 * \
                        needs[key] * np.log(0.5) / np.log(1 - nplayers / nobj)
                else:
                    breaks = -(self.objects_countdown -
                               self.turn) > 300000  # big value
                for x in range(self.size[0]):
                    for y in range(self.size[1]):
                        if breaks[x, y]:
                            self.objects[x][y] = [
                                w for w in self.objects[x][y] if w != key]

    def ppl_cooldown(self):
        diff = {
            key: value -
            self.turn for key,
            value in self.ppl_timeouts.items() if self.turn -
            value > 600}
        if len(diff) > 0:
        ...
        for thing in self.inventory.keys():
            ...
            for person, level in self.ppl_lv.items():
                ...
                if person in self.ppl_inventories:
                    self.level_inventory[thing] += \
                        self.ppl_inventories[person].get(thing, 0)
    ...
    def food_update(self):
        ...
        if self.level_up_cooldown is not None and self.turn - \
                self.level_up_cooldown > 200:
            self.level_up_cooldown = None
            self.level_up = False
    ...
    def voir_processer(self, command: str):
        ...
        for x in range(9999):
            ...
            for y in range(2 * x + 1):
                ...
                coord = coord.tolist()
                self.objects[coord[0] %
                             self.size[0]][coord[1] %
                                           self.size[1]] = contents[w].split()
                self.objects_countdown[coord[0] %
                                       self.size[0], coord[1] %
                                       self.size[1]] = self.turn
                w += 1
    ...
    def generate_message(self, args) -> str:
        if len(self.unsent_commands) > 0 and len(
                self.running_routine) < 10 and self.ok_debt < 10:
            ...
            for y in range(self.size[1]):
                for z in range(-(self.level + 1), self.level + 1):
                    for w in range(-(self.level + 1), self.level + 1):
                        x1 = (x + z) % self.size[0]
                        y1 = (y + w) % self.size[1]
                        mask[x, y] += (1 / (1 + np.abs(z) + np.abs(w))) * (
                            1 - np.exp(-0.015 * (
                                self.turn - self.objects_countdown[x1, y1])))
        return mask
    ...
    def queen_needs_generator(self):
        ...
        for stone in self.inventory.keys():
            if stone == "nourriture":
                continue
            needs[stone] = max(0, needs.get(stone, 0) -
                               self.objects[xpos][ypos].count(stone))
        return needs
    ...
    def drone_buckets(self):
        ...
        buckets = [math.ceil(x / ggat.level_up_party_size(n + 1))
                   for n, x in enumerate(ppl_lv)]
        return buckets
    ...
    def drone_needs(self):
        ...
        reqs = list(map(lambda w: [ggat.level_up_reqs(
            w[0] + 1)] * w[1], enumerate(buckets)))
        reqs = reduce(lambda x, y: x + y, reqs)
        for stone in self.inventory.keys():
            for entry in reqs:
                collect_needs[stone] = collect_needs.get(
                    stone, 0) + entry.get(stone, 0)
    ...
    def call_to_level_up(self):
        ...
        for lvl, nbuckets in enumerate(buckets):
            for _ in range(nbuckets):
                if self.item_ground_count(
                    floor_contents,
                    ggat.level_up_reqs(
                        lvl +
                        1)) and list(
                    known_players.values()).count(
                    lvl +
                    1) >= ggat.level_up_party_size(
                        lvl +
                        1):
                    lvl_players = [
                        player for player,
                        level in known_players.items() if level == lvl +
                        1 and self.ppl_inventories[player]["nourriture"] > 12]
                    if len(lvl_players) < ggat.level_up_party_size(lvl + 1):
                        continue
                    for _ in range(ggat.level_up_party_size(lvl + 1)):
                        who = lvl_players.pop()
