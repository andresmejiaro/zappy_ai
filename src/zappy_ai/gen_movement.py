...
def move_to_distance(
        target: np.array,
        agent: Agent,
        name: None | str = None) -> int:
    tr, _ = move_to_h(target, agent, name)
    return tr


def move_to_h(target: np.array, agent: Agent, name: None |
              str = None) -> tuple[int, BTNode]:
    ...
    target5 = target + DIRECTIONS[0] * \
        agent.size[0] + DIRECTIONS[1] * agent.size[1]
    target6 = target + DIRECTIONS[0] * \
        agent.size[0] - DIRECTIONS[1] * agent.size[1]
    target7 = target - DIRECTIONS[0] * \
        agent.size[0] + DIRECTIONS[1] * agent.size[1]
    target8 = target - DIRECTIONS[0] * \
        agent.size[0] - DIRECTIONS[1] * agent.size[1]
    targets = [
        target,
        target1,
...
def marco_polo_step(agent: Agent):
    if agent.marco_polo_target is not None:
        ...
        target = agent.sound_directionf(
            agent.sound_direction) + agent.pos_at_sound
    else:
    ...
    return ct.GEN(
        lambda x: move_to(
            target,
            x),
        "step towards screamer, marco_polo_step main node")


def marco_polo_follower(agent: Agent):
    do_i_have_direction = ct.LOGIC(
        lambda x: x.sound_direction is not None,
        name="do i have direction")
    # am I there is more layered than before le old sound_direction is zero
    # does not cut it
    is_the_it_inside_the_house = ct.LOGIC(
        lambda x: x.sound_direction == 0,
        name="is the direction 0?")
    is_anyone_here = ct.LOGIC(
        lambda a: a.objects[a.pos[0]][a.pos[1]].count("player") > 0,
        "is_anyone_here")
    am_i_there = ct.GEN(
        lambda x: ct.AND_P(
            [
    ...
    step2 = ct.OR([ct.AND([am_i_there], "if we are there scream"),
                  step3], "if we are not there move ")
    step1 = ct.OR([ct.AND([do_i_have_direction, step2]),
                   ct.ALWAYS_F(gen_interaction("inventaire"),
                               "fallback marco polo step")],
